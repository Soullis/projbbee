import rclpy
from rclpy.node import Node
from std_msgs.msg import String
import ambiente as am

class ObjDetectorPub(Node):
    def __init__(self, max_obs):
        super().__init__("obj_detector_pub")

        self.publisher_ =self.create_publisher(String, 'detection_topic', 10)

        timer_period = 0.5

        self.timer = self.create_timer(timer_period, self.timer_callback)

        self.max_obs = max_obs

    def timer_callback(self):
        msg = String()
        if am.obstaculos[am.current_obs].limits(8)[0] <= am.drone.pos[0] <= am.obstaculos[am.current_obs].limits(8)[1] and am.drone.obj_detector_range[0] <= am.obstaculos[am.current_obs].posY <= am.drone.obj_detector_range[1]:
            msg.data = f"{am.obstaculos[self.current_obs].posY - am.drone.pos[2]}"
        else:
            msg.data = "-1"

        self.publisher_.publish(msg)


def main(args=None):
    rclpy.init(args=args)

    obj_detector_pub = ObjDetectorPub()
    
    rclpy.spin(obj_detector_pub)

    obj_detector_pub.destroy_node()
    rclpy.shutdown()

if __name__ == '__main__':
    main()
